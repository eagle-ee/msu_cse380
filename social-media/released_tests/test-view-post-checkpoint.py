...
try:
	...
	URLView = "http://127.0.0.1:5000/view_post/37"
	VIEWPARAMS = {'title': True, 'body': True, 'tags': True, 'owner': True, 'likes': True}
	r_view = requests.get(url = URLView, params = VIEWPARAMS, headers={'Authorization': 'eyJhbGciOiAiSFMyNTYiLCAidHlwIjogIkpXVCJ9.eyJ1c2VybmFtZSI6ICJjaGVhdGVyMiIsICJhY2Nlc3MiOiAiVHJ1ZSJ9.df06dea4610cb3a49403845e6e8f91e15714d30abb5324d7ba09adca1a63cb0a'})
	view_data = r_view.json()

	expected = json.dumps({'status': 1,'data': {'title': 'If I Did It: Confessions of the Cheater','body': 'I don’t understand what I did wrong except live a life that everyone is jealous of','tags': ['#ifyaaintcheatingyaainttrying', '#cheatercheaterpumpkineater'],'owner': 'cheater','likes': 1}})
	expected_dict = json.loads(expected)


	for x in expected_dict['data']:
		if x == 'tags':
			if len(expected_dict['data'][x]) != len(view_data['data'][x]):
				quit()
			else:
				for tag in view_data['data'][x]:
					if tag not in view_data['data'][x]:
						quit()
		elif expected_dict['data'][x] != view_data['data'][x]:
			quit()
