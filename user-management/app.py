...
def create_user():
	...
	cursor.execute("INSERT INTO users (username, first_name, last_name, email_address, password, salt)\
				   VALUES (?,?,?,?,?,?);", (uname, first, last, email, hash, salt))
...
def generateJWT(username, access):
	"""
...
def update():
	...
	try:
		...
		with open('key.txt', 'r') as key:
			signatureKey = key.read().strip()
		checkSig = hmac.new(signatureKey.encode('utf-8'), f'{header}.{payload}'.encode('utf-8'), hashlib.sha256).hexdigest()
...
def view():
	...
	try:
		...
		uname = payloadDict.get("username")
